...
class GameManagerViewModel: ObservableObject{
    ...
    static var quizQuestions: [QuizQuestion] = quizQuestionsDb.shuffled()
    
    static func createGameModel(i: Int) -> Quiz {
        return Quiz(currentQuestionIndex: i, quizQuestion: quizQuestions[i])
    }
    ...
    func verifyAnswer(selectedOption: QuizOption){
        for index in model.quizQuestion.optionsList.indices{
            model.quizQuestion.optionsList[index].isMatched = false
            model.quizQuestion.optionsList[index].isSelected = false
        }
        if let index = model.quizQuestion.optionsList.firstIndex(where: {$0.optionId == selectedOption.optionId}){
            if selectedOption.optionId == model.quizQuestion.answer {
                model.quizQuestion.optionsList[index].isMatched = true
                model.quizQuestion.optionsList[index].isSelected = true
                
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5){
                    if GameManagerViewModel.currentIndex < GameManagerViewModel.quizQuestions.count - 1{
